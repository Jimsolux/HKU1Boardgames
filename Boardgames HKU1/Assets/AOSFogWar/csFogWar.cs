...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        private void UpdateFogPlaneTextureBuffer()
        {
            ...
            // Adjust the interpolation speed outside the loop
            float lerpFactor = fogLerpSpeed * Time.deltaTime;

            // Ensure buffer and target texture sizes are compatible
            if (bufferPixels.Length != targetPixels.Length)
            {
                Debug.LogError("Texture sizes are different. Cannot perform fog update.");
                return;
            }
            ...
            {
                bufferPixels[i] = Color.Lerp(bufferPixels[i], targetPixels[i], lerpFactor);
            }
